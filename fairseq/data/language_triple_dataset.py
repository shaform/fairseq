...
def collate(
    ...
    # target source
    tgt_tokens = merge('target_source', left_pad=left_pad_source)
    tgt_tokens = tgt_tokens.index_select(0, sort_order)
    tgt_lengths = torch.LongTensor([s['target_source'].numel() for s in samples]).index_select(0, sort_order)
...
class LanguageTripleDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.remove_eos_from_source:
            ...
            eos = self.tgt_dict.eos() if self.tgt_dict else self.src_dict.eos()
            if self.tgt[index][-1] == eos:
                tgt_src_item = self.tgt[index][:-1]

        example = {
            'id': index,
            'source': src_item,
            'target_source': tgt_src_item,
            'target': tgt_item,
        }
        return example
